...
The content of this deliberation follows IHL's targeting principles. Rather than a single button press, the workflow should decompose into separate confirmations: \textit{distinction} (is this a valid military target?), \textit{proportionality} (is the anticipated military advantage proportionate to the expected civilian harm?), and \textit{precaution} (have all measures to minimise civilian harm been made?). Requiring a confirmation per principle implements the legal standard identified by \citet{ootlaws}, and prevents a multi-step judgement into a single button press.

Moreover, \citet{parasuraman2010complacency} demonstrate that automation complacency is most pronounced when automation is highly and consistently reliable and when operators face competing task demands. The interface must actively interrupt this cycle by requiring engagement checks in the explanation layer: an operator should not be able to click through the IHL checklist without first reviewing the contrastive explanations and feature weights. This creates a forced interaction with the XAI output, preventing the explanation layer from becoming an obstacle to be bypassed rather than a genuine deliberative step.

\citet{amershi2019guidelines}'s human-AI interaction guidelines further require that the system communicate how well it can perform, calibrating displayed confidence to actual reliability and surfacing uncertainty explicitly. This directly counters automation bias by ensuring that operator trust is proportionate to system performance rather than assumed.
...
A mandatory pre-deployment ethical impact assessment (EIA) should be grounded in UNESCO's recommendation against delegating life-or-death decisions to AI~\citep{unesco2021recommendation}. This ensures that human rights implications are systematically reviewed before deployment. Binding policy-level requirements are necessary to prevent the kind of gaps in governance identified in \citet{khan2021ethicsaisystematicliterature}'s \textbf{P-03} accountability and \textbf{P-05} autonomy, regarding the absence of accountability mechanisms and the erosion of human agency in AI systems.
...
\citet{info12090385} integrate this approach into the governance layer of the CHOF: we argue it should not remain an optional design choice, but rather an operational legal requirement.
