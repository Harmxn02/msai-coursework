...
1. Eerst gebruik ik De Morgan op het eerst complement: $(A \cap B) \cup B = ()$

\[
{((B \cap A^c) \cup {B}^c)}^c = {(B \cap A^c)}^c \cap {(B^c)}^c
\]

En ${(B^c)}^c = (B)$ dus dan wordt het:

\[
{((B \cap A^c) \cup {B}^c)}^c = {(B \cap A^c)}^c \cap B
\]

2. Dan gebruik ik nog eens De Morgan op het linkerdeel:
...
3. Dan gebruik ik distributiviteit:

\[
(B^c \cup A) \cap B = (B^c \cap B) \cup (A \cap B)
\]


Het linkerdeel is een lege verzameling omdat de doorsnede van $B$ met hetgene wat niet in $B$ zit niets kan zijn.
