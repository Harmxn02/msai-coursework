...
Om te bewijzen dat $\preceq$ een partiele ordening is, moeten we aantonen dat het reflexief, antisymmetrisch en transitief is.

\underline{reflexief}:
$a \preceq a$

voor $n = n^1$ klopt de relatie, dus $a \preceq a$


\underline{antisymmetrisch}:
$a \preceq b \neq b \preceq a$

de relatie is antisymmetrisch want als $n \preceq m$ en $m \preceq n$, dan is $m = n^a$ en $n = m^b$, met a, b elk natuurlijke getallen boven 0.

Hieruit volgt dat $n \leq m$ en $m \leq n$, wat betekent $m = n$

\underline{transitief}:
$\text{als } a \preceq b \text{ en } b \preceq c \text{ dan } a \preceq c$

De relatie is transitief want uit $n \preceq m$ en $m \preceq p$ volgt $m = n^a$ en $p = m^b$, waaruit volgt $p = {(n^a)}^b = {n^a}^b$ dus $n \preceq p$

Hier is $p$ dus $n$ tot een exponent = onze relatie
